...
class VenueMetric(Base):
    """Generic dimensional table for venue-year-metric combinations"""
    __tablename__ = "venue_metrics"

    id = Column(Integer, primary_key=True, autoincrement=True)
    venue = Column(String(500), nullable=False)  # Venues can have longer names
    year = Column(Integer, nullable=False)
    metric_type = Column(String(50), nullable=False)  # 'total', 'has_abstract', 'has_full_text', etc.
    count = Column(Integer, nullable=False)
    ...
    # Indexes for fast querying + unique constraint for upserts
    __table_args__ = (
        Index("ix_venue_metrics_venue", "venue"),
        Index("ix_venue_metrics_year", "year"),
        Index("ix_venue_metrics_metric_type", "metric_type"),
        Index("ix_venue_metrics_venue_year_metric", "venue", "year", "metric_type", unique=True),
    )
