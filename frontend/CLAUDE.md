...
- Global styles in `src/styles/` (base.css, reset.css, theme.css, etc.)
...
### Build Configuration
- **Vite** config includes custom path aliases: `$data` → `src/data`, `$styles` → `src/styles`, `$stories` → `src/lib/stories`
...
- SvelteKit remote functions used for backend API integration via `data.remote.js` files
- Stories can define query functions with Valibot validation
- API calls go to backend FastAPI server (typically on port 3001)
- Frontend maintains static data files in `static/data/` for high-performance client-side processing
