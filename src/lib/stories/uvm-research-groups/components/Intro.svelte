...
<section id="story" class="story">  
  ...
  <p>We denote faculty with or without research groups in yellow and green in typical UVM fashion. Highlighted in red, we have faculty without openAlex identifiers, which we use as our main academic database for author metadata. As can be seen below, these are mostly faculty engaged in the arts: theater, music, dance, art history.</p>
  <p>Last but not least, we chose to represent perceived sex as binary shapes. Unfortunately, we didn't record faculty gender identity, and even if we did we would be hesitant to share the data here. The assessment of perceived sex was done while annotating faculty with research groups or not, and was based on perceived information about faculty; mostly their faculty headshots. We felt it was important to represent that category to better understand gendered patterns in science, and in particular at UVM.</p>
  <p>Now, we can see that about one third of faculty have research groups, defined here as any claim from faculty to have some kind of research group on the Internet. This is a somewhat restrictive definition of research groups, but that is somewhat aligned with the idea of groups in collective action theory. That is, it is a definition of groups that stems from individuals, here principal investigators (PIs), recognizing themselves as such. Now, we can look at how this proportion changes when stratified by  <span class="grouping-controls">
    <button 
...
<p>Already these waffle charts are saying something interesting. Looking at colleges, we note that roughly half of faculty in the College of Medicine and CEMS have research groups. Looking at perceived sex, we can see there are more male PIs with research groups ({(25/30 * 100).toFixed(2)}% of PIs are also males) than women (versus {(27/40 * 100).toFixed(2)}% of non-PIs are perceived as males in the College of Medicine. Combining both numbers, we find that only {(16/69*100).toFixed(2)}% of faculty in the College of Medicine are women).</p>

<p>CEMS has an even lower proportion with only {3/24*100}% PIs perceived as women. We should be careful in interpreting those numbers at face value though. Part of our definition of research group is that the faculty is claiming publicly (on the Internet) to have a group. By that definition, Professor <a href="https://drizzo.w3.uvm.edu/">Donna M. Rizzo</a>, for example, has more graduate students than some PIs, but she never explicitly says that this is "her research group". We had to draw the line somewhere. Future modeling work could potentially address those pitfalls, but the <a href="https://arxiv.org/abs/2507.02758">ontology of groups</a> is famously hard to pin down.</p>

<p>The College of Agriculture and Life Sciences (CALS) and the Rubenstein School of Environment have higher proportions of faculty with groups and are closer to gender equality! Finally, as one would expect, the College of Arts and Sciences (CAS) has fewer PIs, with about {(19/80)*100}% of faculty having groups. In the College of Nursing and Health Sciences, Education and Social Services, and Business, faculty with research groups are in the minority, hinting at different epistemic cultures at UVM.</p>

<p>Looking at the department level, we can see that most groups in CAS are either in Psychological Science, Biology, or Chemistry. In CALS, it is also heterogeneous with CDAE having no research groups, while in other departments the majority of faculty do have groups.</p>
